...
// Our own sort function in ES5
carModelsSorted = carModels.sort(function(a,b) {
	if (a < b) return -1;
	if (a > b) return 1;
});
// Our own sort function in ES6
carModelsSorted = carModels.sort((a, b) => {
	if (a < b) return -1;
	if (a > b) return 1;
});
// Our own sort function in ES6 with ternary operator
carModelsSorted = carModels.sort((a, b) => {
	(a < b) ? -1 : 1;
});

console.log(JSON.stringify(carModelsSorted));
...
// Implementation 3
//carYears = Array.from(new Set(carYears));

// Implementation 4
// carYears = carYears.filter(function onlyUnique(value, index, self) {
...
// for (let i = 0; i < inventory.length; i++) {
// 	const element = inventory[i];
// 	if(carYears.indexOf(inventory[i].car_year) === -1) {
// 		carYears.push(inventory[i].car_year);
// 	}
// }
